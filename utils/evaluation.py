...
def get_OSCR_AUROC(logit_k, logit_u, labels_k):
    x1, _ = torch.max(logit_k, 1)
    x2, _ = torch.max(logit_u, 1)
    label_prob = torch.cat((x1, x2), dim=0)
    label_binary = torch.cat((torch.ones(x1.size(0)), torch.zeros(x2.size(0))), dim=0)
    label_binary = label_binary.cpu().numpy()
    label_prob = label_prob.cpu().numpy()
    logit_k = logit_k.cpu().numpy()
    logit_u = logit_u.cpu().numpy()
    labels_k = labels_k.cpu().numpy()
    auroc_score = roc_auc_score(label_binary, label_prob)
    oscr_score = compute_oscr(logit_k, logit_u, labels_k)

    return auroc_score, oscr_score